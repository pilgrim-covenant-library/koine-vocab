...
import { getQuestionsForHW8Section, type HW8SectionId } from '@/data/homework/hw8-questions';
import { HW8_SECTION_META, type MCQQuestion, type TranslationQuestion } from '@/types/homework';
...
const SECTION_IDS: HW8SectionId[] = [1, 2, 3, 4, 5, 6];
...
export default function HW8SectionPage() {
  ...
  const sectionId = (isValidSectionId ? parsedId : 1) as HW8SectionId;

  const { user } = useAuthStore();

  // Use targeted selectors to prevent over-subscription
  const sectionProgress = useHomeworkStore(
    useShallow((state) => state.homework8.sections[sectionId] ?? {
  ...
  const sectionStatuses = useHomeworkStore(
    useShallow((state) =>
      Object.fromEntries(
        SECTION_IDS.map((id) => [id, state.homework8.sections[id]?.status ?? 'not_started'])
      ) as Record<HW8SectionId, 'not_started' | 'in_progress' | 'completed'>
    )
  );

  // Actions don't cause re-renders - select individually
  const startSection8 = useHomeworkStore((state) => state.startSection8);
  const submitAnswer8 = useHomeworkStore((state) => state.submitAnswer8);
  const nextQuestion8 = useHomeworkStore((state) => state.nextQuestion8);
  const previousQuestion8 = useHomeworkStore((state) => state.previousQuestion8);
  const completeSection8 = useHomeworkStore((state) => state.completeSection8);
  const completeHomework8 = useHomeworkStore((state) => state.completeHomework8);
  const syncToCloud8 = useHomeworkStore((state) => state.syncToCloud8);
  ...
  const questions = useMemo(() => getQuestionsForHW8Section(sectionId), [sectionId]);
  ...
  useEffect(() => {
    if (!isValidSectionId) {
      ...
      router.replace('/homework/hw8');
      return;
    }
    startSection8(sectionId);
  }, [sectionId, isValidSectionId, params.id, startSection8, router]);
  ...
  const handleSubmit = useCallback(() => {
    ...
    submitAnswer8(sectionId, currentQuestion.id, selectedOption, correct);

    setIsCorrect(correct);
    setShowFeedback(true);

    debouncedSync();
  }, [currentQuestion, selectedOption, sectionId, submitAnswer8, debouncedSync]);
  ...
  const handleTranslationSubmit = useCallback(() => {
    ...
    submitAnswer8(sectionId, currentQuestion.id, translationInput, correct);
    ...
    debouncedSync();
  }, [currentQuestion, translationInput, sectionId, submitAnswer8, debouncedSync]);

  const handleNext = () => {
    if (!nextQuestion8(sectionId)) {
      // Last question
    }
  };

  const handlePrevious = () => {
    previousQuestion8(sectionId);
  };

  const handleComplete = async () => {
    completeSection8(sectionId);
    ...
    if (sectionId === 6) {
      completeHomework8();
      router.push('/homework/hw8/complete');
    } else {
      router.push(`/homework/hw8/section/${sectionId + 1}`);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <FloatingHelpButton currentSection={sectionId} homeworkId="hw8" />
    </div>
  );
}
