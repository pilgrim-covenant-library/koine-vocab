...
import { ArrowLeft, BookOpen, CheckCircle, Clock, PlayCircle, GraduationCap, Lock } from 'lucide-react';
...
export default function HomeworkPage() {
  const { homework1, homework2, homework3, homework4, homework5, homework6, homework7, homework8, finalExam, getOverallProgress, getOverallProgress2, getOverallProgress3, getOverallProgress4, getOverallProgress5, getOverallProgress6, getOverallProgress7, getOverallProgress8, getOverallProgressFE } = useHomeworkStore();
  ...
  const progress8 = getOverallProgress8();
  const progressFE = getOverallProgressFE();
        ...
        <div className="space-y-6">
          ...
          <Card className="overflow-hidden">
            <div
              className={cn(
                'h-2',
                homework8.status === 'completed'
                  ? 'bg-green-500'
                  : homework8.status === 'in_progress'
                  ? 'bg-primary'
                  : 'bg-muted'
              )}
              style={{
            ...
            <CardHeader>
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <CardTitle className="text-xl">Homework 8</CardTitle>
                    <CardDescription>
                      3rd Declension, Adjectives & Infinitives
                    </CardDescription>
                  </div>
                </div>
                <span
            ...
            <CardContent className="space-y-4">
              <p className="text-sm text-muted-foreground">
                For students learning 3rd declension morphology and Greek adjective/infinitive forms. This homework covers:
              </p>
              <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
                <li className="flex items-center gap-2">
                  <span className="w-5 h-5 rounded-full bg-primary/10 text-primary text-xs flex items-center justify-center font-medium">
                ...
                <div className="pt-2 border-t">
                  ...
                  {homework8.status === 'completed' && (
                    <div className="flex items-center justify-between text-sm mt-1">
                      <span className="text-muted-foreground">Score</span>
                      <span className="font-medium">
                        {homework8.totalScore}/{homework8.totalPossible} (
                        {Math.round(
                          (homework8.totalScore / homework8.totalPossible) * 100
                        )}
                        %)
                      </span>
                    </div>
              ...
              <div className="pt-2">
                {homework8.status === 'completed' ? (
                  <Link href="/homework/hw8/complete">
                    <Button className="gap-2">
                      <CheckCircle className="w-4 h-4" />
                      View Results
                    </Button>
            ...
            <CardHeader>
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <CardTitle className="text-xl">Final Exam</CardTitle>
                    <CardDescription>Comprehensive Assessment — 100 questions</CardDescription>
                  </div>
                </div>
                <span
                  className={cn(
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 gap-2 text-sm">
                  <div className="flex items-center gap-2 text-muted-foreground">
                    <span className="w-4 h-4 rounded-full bg-muted flex items-center justify-center text-[10px] font-medium">1</span>
                    Grammar Understanding (50 questions)
                  </div>
                  <div className="flex items-center gap-2 text-muted-foreground">
                    <span className="w-4 h-4 rounded-full bg-muted flex items-center justify-center text-[10px] font-medium">2</span>
                    Vocabulary (30 questions)
                  </div>
                  <div className="flex items-center gap-2 text-muted-foreground">
                    <span className="w-4 h-4 rounded-full bg-muted flex items-center justify-center text-[10px] font-medium">3</span>
                    Verse Translation (20 questions)
                  </div>
                </div>

                {finalExam.status !== 'not_started' && (
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      <span className="text-muted-foreground">Progress</span>
                      <span className="font-medium">{progressFE.completed}/3 sections</span>
                    </div>
                    {finalExam.status === 'completed' && (
                      <div className="flex justify-between text-sm">
                        <span className="text-muted-foreground">Score</span>
                        <span className="font-medium text-green-600 dark:text-green-400">
                          {finalExam.totalScore}/{finalExam.totalPossible} ({Math.round((finalExam.totalScore / finalExam.totalPossible) * 100)}%)
                        </span>
                      </div>
                    )}
                  </div>
