...
import { getQuestionsForHW5Section, type HW5SectionId } from '@/data/homework/hw5-questions';
import { HW5_SECTION_META, type MCQQuestion, type TranslationQuestion } from '@/types/homework';
...
export default function HW5SectionPage() {
  ...
  const sectionId = (isValidSectionId ? parsedId : 1) as HW5SectionId;

  const { user } = useAuthStore();

  // Use targeted selectors to prevent over-subscription
  const sectionProgress = useHomeworkStore(
    useShallow((state) => state.homework5.sections[sectionId] ?? {
  ...
  const sectionStatuses = useHomeworkStore(
    useShallow((state) =>
      Object.fromEntries(
        SECTION_IDS.map((id) => [id, state.homework5.sections[id]?.status ?? 'not_started'])
      ) as Record<HW5SectionId, 'not_started' | 'in_progress' | 'completed'>
    )
  );

  // Actions don't cause re-renders - select individually
  const startSection5 = useHomeworkStore((state) => state.startSection5);
  const submitAnswer5 = useHomeworkStore((state) => state.submitAnswer5);
  const nextQuestion5 = useHomeworkStore((state) => state.nextQuestion5);
  const previousQuestion5 = useHomeworkStore((state) => state.previousQuestion5);
  const completeSection5 = useHomeworkStore((state) => state.completeSection5);
  const completeHomework5 = useHomeworkStore((state) => state.completeHomework5);
  const syncToCloud5 = useHomeworkStore((state) => state.syncToCloud5);
  ...
  const questions = useMemo(() => getQuestionsForHW5Section(sectionId), [sectionId]);
  ...
  useEffect(() => {
    if (!isValidSectionId) {
      ...
      router.replace('/homework/hw5');
      return;
    }
    startSection5(sectionId);
  }, [sectionId, isValidSectionId, params.id, startSection5, router]);
  ...
  const handleSubmit = useCallback(() => {
    ...
    submitAnswer5(sectionId, currentQuestion.id, selectedOption, correct);

    setIsCorrect(correct);
    setShowFeedback(true);

    debouncedSync();
  }, [currentQuestion, selectedOption, sectionId, submitAnswer5, debouncedSync]);
  ...
  const handleTranslationSubmit = useCallback(() => {
    ...
    debouncedSync();
  }, [currentQuestion, translationInput, sectionId, submitAnswer5, debouncedSync]);

  const handleNext = () => {
    if (!nextQuestion5(sectionId)) {
      // Last question
    }
  };

  const handlePrevious = () => {
    previousQuestion5(sectionId);
  };

  const handleComplete = async () => {
    completeSection5(sectionId);
    ...
    if (sectionId === 6) {
      ...
      router.push(`/homework/hw5/section/${sectionId + 1}`);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <FloatingHelpButton currentSection={sectionId} homeworkId="hw5" />
    </div>
  );
}
