...
export default function HW7CompletePage() {
  ...
  useEffect(() => {
    const finalize = async () => {
      if (homework7.status !== 'completed' && progress.completed === 6) {
        completeHomework7();
      }
    ...
    finalize();
  }, [homework7.status, progress.completed, completeHomework7, user, syncToCloud7, submitResult7, hasSynced]);

  // Calculate grade (guard against division by zero)
  const percentage = homework7.totalPossible > 0
    ? Math.round((homework7.totalScore / homework7.totalPossible) * 100)
    : 0;
  const getGrade = () => {
  ...
  const handleRetry = async () => {
    ...
    router.push('/homework/hw7');
  };

  const handleRedoSection = async (sectionId: HW7SectionId) => {
      ...
      <main className="container mx-auto px-4 py-12 max-w-2xl">
        <div className="space-y-8">
          ...
          <div className="text-center space-y-4">
            ...
            <h1 className="text-3xl font-bold">Homework 7 Complete!</h1>
            <p className="text-muted-foreground">
              Great job completing Homework 7: Middle/Passive Participles, Perfect Tense, Pluperfect & Subjunctive
            </p>
            {/* Sync indicator */}
            {user && (
            ...
            <CardContent className="pt-8 pb-6 text-center">
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-center gap-8 py-4 border-y">
                  <div>
                    <p className="text-3xl font-bold">{homework7.totalScore}</p>
                    <p className="text-sm text-muted-foreground">Correct</p>
                  </div>
                  <div className="text-3xl text-muted-foreground">/</div>
                  <div>
                    <p className="text-3xl font-bold">{homework7.totalPossible}</p>
              ...
              <div className="space-y-2">
                {percentage >= 90 && (
                  <>
                    ...
                    <p className="text-sm text-muted-foreground">
                      You have demonstrated a strong understanding of the perfect tense, pluperfect,
                      middle/passive participles, and the subjunctive mood. These are foundational
                      for reading New Testament Greek with precision!
                    </p>
                  </>
                )}
                {percentage >= 70 && percentage < 90 && (
                  <>
                    ...
                    <p className="text-sm text-muted-foreground">
                      You're developing solid skills with the perfect tense system and subjunctive mood.
                      Review any sections where you scored below 80%, paying special attention to
                      reduplication patterns and the lengthened connecting vowels of the subjunctive.
                    </p>
                  </>
                )}
                {percentage < 70 && (
                  <>
                    ...
                    <p className="text-sm text-muted-foreground">
                      The perfect tense and subjunctive mood are challenging but essential. Review the
                      paradigm charts for perfect active/middle-passive forms, the pluperfect endings,
                      and subjunctive vowel lengthening, then try again when ready.
                    </p>
                  </>
                )}
              </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <Link href="/homework/hw7" className="w-full sm:w-auto">
              <Button className="gap-2 w-full">
                Back to HW7 Overview
