...
import { getQuestionsForHW6Section, type HW6SectionId } from '@/data/homework/hw6-questions';
import { HW6_SECTION_META, type MCQQuestion, type TranslationQuestion } from '@/types/homework';
...
const SECTION_IDS: HW6SectionId[] = [1, 2, 3, 4, 5, 6];
...
export default function HW6SectionPage() {
  ...
  const sectionId = (isValidSectionId ? parsedId : 1) as HW6SectionId;

  const { user } = useAuthStore();

  // Use targeted selectors to prevent over-subscription
  const sectionProgress = useHomeworkStore(
    useShallow((state) => state.homework6.sections[sectionId] ?? {
  ...
  const sectionStatuses = useHomeworkStore(
    useShallow((state) =>
      Object.fromEntries(
        SECTION_IDS.map((id) => [id, state.homework6.sections[id]?.status ?? 'not_started'])
      ) as Record<HW6SectionId, 'not_started' | 'in_progress' | 'completed'>
    )
  );

  // Actions don't cause re-renders - select individually
  const startSection6 = useHomeworkStore((state) => state.startSection6);
  const submitAnswer6 = useHomeworkStore((state) => state.submitAnswer6);
  const nextQuestion6 = useHomeworkStore((state) => state.nextQuestion6);
  const previousQuestion6 = useHomeworkStore((state) => state.previousQuestion6);
  const completeSection6 = useHomeworkStore((state) => state.completeSection6);
  const completeHomework6 = useHomeworkStore((state) => state.completeHomework6);
  const syncToCloud6 = useHomeworkStore((state) => state.syncToCloud6);
  ...
  const questions = useMemo(() => getQuestionsForHW6Section(sectionId), [sectionId]);
  const currentQuestion = questions[sectionProgress.currentIndex];
  const meta = HW6_SECTION_META[sectionId];
  ...
  useEffect(() => {
    if (!isValidSectionId) {
      ...
      router.replace('/homework/hw6');
      return;
    }
    startSection6(sectionId);
  }, [sectionId, isValidSectionId, params.id, startSection6, router]);
  ...
  const handleSubmit = useCallback(() => {
    ...
    submitAnswer6(sectionId, currentQuestion.id, selectedOption, correct);

    setIsCorrect(correct);
    setShowFeedback(true);

    debouncedSync();
  }, [currentQuestion, selectedOption, sectionId, submitAnswer6, debouncedSync]);
  ...
  const handleTranslationSubmit = useCallback(() => {
    ...
    submitAnswer6(sectionId, currentQuestion.id, translationInput, correct);
    ...
    debouncedSync();
  }, [currentQuestion, translationInput, sectionId, submitAnswer6, debouncedSync]);

  const handleNext = () => {
    if (!nextQuestion6(sectionId)) {
      // Last question
    }
  };

  const handlePrevious = () => {
    previousQuestion6(sectionId);
  };

  const handleComplete = async () => {
    completeSection6(sectionId);
    ...
    if (sectionId === 6) {
      completeHomework6();
      router.push('/homework/hw6/complete');
    } else {
      router.push(`/homework/hw6/section/${sectionId + 1}`);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <FloatingHelpButton currentSection={sectionId} homeworkId="hw6" />
    </div>
  );
}
