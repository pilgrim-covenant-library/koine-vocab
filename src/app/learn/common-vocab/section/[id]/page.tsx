...
import { X, RotateCcw, ChevronRight, Trophy, ThumbsUp, Zap, Keyboard, Cloud, CloudOff, Loader2, Crown, ArrowLeft } from 'lucide-react';
import { useUserStore } from '@/stores/userStore';
import { useSessionStore } from '@/stores/sessionStore';
import { useAuthStore } from '@/stores/authStore';
import { syncProgressToCloud } from '@/lib/firebase';
import { getCommonVocabSection, COMMON_VOCAB_SECTION_META, WORDS_PER_SECTION, type CommonVocabSectionId } from '@/lib/commonVocab';
...
export default function CommonVocabSectionPage() {
  const router = useRouter();
  const params = useParams();
  const parsedId = parseInt(params.id as string, 10);
  const isValidId = !isNaN(parsedId) && VALID_SECTION_IDS.has(parsedId);
  const sectionId = (isValidId ? parsedId : 1) as CommonVocabSectionId;

  const { user } = useAuthStore();
  const { stats, progress, reviewWord, initializeWord, checkAndUnlockAchievements, recordSession, recordCommonVocabSectionScore } = useUserStore();
  const {
    isActive,
    mode,
    startSession,
  ...
  const sectionMeta = COMMON_VOCAB_SECTION_META[sectionId];

  useEffect(() => {
    setMounted(true);
  }, []);

  // Redirect if invalid section ID
  useEffect(() => {
    if (mounted && !isValidId) {
      router.replace('/learn/common-vocab');
    }
  }, [mounted, isValidId, router]);
  ...
  // Initialize session with section words
  useEffect(() => {
    if (!mounted || !isValidId) return;
    ...
    if (!isActive) {
      const sectionWords = getCommonVocabSection(sectionId);
      const sessionWords = shuffle([...sectionWords]);
  ...
  }, [mounted, isValidId, isActive, mode, words, questions.length, sectionId, initializeWord, startSession, endSession]);
  ...
  const handleNext = useCallback(() => {
    if (currentIndex < words.length - 1) {
      ...
      recordCommonVocabSectionScore(sectionId, sessionStatsData.correct, sessionStatsData.total);

      setSessionComplete(true);
    }
  }, [currentIndex, words.length, nextWord, getSessionStats, checkAndUnlockAchievements, sectionId, recordCommonVocabSectionScore]);
  ...
  const handleEndSession = async () => {
    ...
    router.push('/learn/common-vocab');
  };
          ...
          <Card className="max-w-md w-full overflow-hidden">
            ...
            <CardContent className="py-8 text-center">
              ...
              <p className="text-sm text-muted-foreground mb-1">
                Section {sectionId}: {sectionMeta.title}
              </p>
        ...
        <div className="container mx-auto px-4 py-3">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-1">
              <Button variant="ghost" size="icon" onClick={handleEndSession} aria-label="Back to sections" className="h-8 w-8">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium">
                Section {sectionId} — {sessionProgress.current} / {sessionProgress.total}
              </span>
            </div>
            <div className="flex items-center gap-1">
              <Button variant="ghost" size="icon" onClick={handleRetry} aria-label="Retry section" className="h-8 w-8" title="Restart section">
                <RotateCcw className="w-4 h-4" />
              </Button>
              <Button variant="ghost" size="icon" onClick={handleEndSession} aria-label="Exit session" className="h-8 w-8">
                <X className="w-5 h-5" />
              </Button>
            </div>
          </div>
